...
do_download() {
  ...
  # Most of the Workstation bits still live in ChefDK - grab that here
  pushd $PLAN_CONTEXT/..
  rm -rf "chef-dk"
  git clone "https://github.com/chef/chef-dk" "chef-dk"
  pushd chef-dk
  # TODO - remove this before merge - should we used a pinned/auto-updated version?
  git checkout "mp/ws-hab-integration"
  # Build the gem as well - unpack below will expect this gem to exist
...
export HAB_WS_GIT_BIN_DIR=$(hab pkg path core/git)/bin
export HAB_WS_EMBEDDED_DIR=$pkg_prefix/ruby-bin:$(hab pkg path core/bundler):$(hab pkg path core/ruby/$RUBY_VERSION)/bin:$(hab pkg path core/git)/bin
